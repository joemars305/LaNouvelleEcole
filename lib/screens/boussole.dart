...
<<<<<<< HEAD
  ...
  void createNotificationOnce(
    ...
    /// sauvegarde les infos concernant la choses a faire
    noteToFutureSelf.combienDeFois = UNE_SEULE_FOIS;
    userReport.notesToFutureSelf.add(noteToFutureSelf);
    userReport.save();

    /// crée une notif
    Notifier notifier = Provider.of<Notifier>(context);
    DateTime when = noteToFutureSelf.getTimeOfAction();
    String title = "Voici ta mission, soldat:";
    String body = noteToFutureSelf.name;
    notifier.scheduleNotificationOnce(when, title, body);
